...
<br>
systemd searches units in following locations in the order of highest priority to lowest

1. /etc/systemd/system
2. /run/systemd/system/
3. /usr/lib/systemd/system or /lib/systemd/system/

>> HOW TO EDIT EXISTING SERVICES

use systemctl to edit exisiting services
`sudo systemctl edit myapp.service` - edits the service by creating a override.conf and keeping the edited configurations

If an existing service (vendor provided services) are to be modified e.g. Restart=`no` to be modified as `always`
create a `.conf` file and just keep configurations that needs to be modified. e.g. <br>

`create /etc/systemd/system/myapp.service.d/override.conf`
...
create folder <br>
`/etc/systemd/system/myapp.service.d/` <br>
drop different override.confs here <br>
e.g. `limits.conf`, `RestartTimer.conf` <br>
e.g. limits.conf - <br>
[Service]
Restart=always
